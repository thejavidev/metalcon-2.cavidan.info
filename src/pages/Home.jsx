...
import { about, characteristics, homeBanner, myprojects, myservices } from '../components/fadedata/fakedata';
      ...
      >
        ...
        >
          {
            homeBanner && homeBanner?.map((cur, i) => (
              <SwiperSlide key={i} className='relative sliderItem'>
                <LazyLoadImage src={cur?.img} className='w-full h-[650px] md:h-[450px]' />
                <div className="flex justify-center items-center flex-col top-[0] md:top-[40px] h-full absolute w-full">
                  <div className='text-[#fff] text-center' >test</div>
                ...
                <SwiperSlide key={i} className='services servicesa p-[30px] h-[280px] relative'>
                  <div className="flex justify-between items-center flex-col h-full w-full">
                   ...
                   >{cur?.name}</div>
                    ...
                    <p className='font-[200] text-[14px] text-center pb-[20px] text-[#fff]' >{cur?.title}</p>
        ...
        <section id="about" className='pt-[50px] pb-[50px] bg-[--bg]'>
          <Container>
            <div className="w-full flex justify-end items-end  md:mb-4 md:items-center md:justify-start">
              <h2 className='text-[35px] lg:text-[25px] font-bold text-[#3498db] '>test <span className='text-[--text] font-[200]'>test</span></h2>
            </div>
            <div className="w-full flex items-center mb-[30px]">
              <LazyLoadImage src={line} className='h-[2px] mr-[20px]' />
              <h2 className='text-[20px] font-bold text-[#3498db] uppercase  '>test <span className='text-[--text] '>test</span></h2>
            </div>
            <Row className='items-center '>
              ...
              <Col lg={6} className='flex items-center  flex-col'>
                <div className="w-full text-justify line-clamp-5 lg:line-clamp-3 mb-4 text-[--text]">{about?.text}</div>
        ...
        <section id="spesfics" className='pt-[50px] pb-[50px] bg-[--bg2]'>
          <Container>
            ...
            <div className="w-full flex items-center mb-[30px]">
              <LazyLoadImage src={line} className='h-[2px] mr-[20px]' />
              <h2 className='text-[20px] font-bold text-[--text] uppercase'>test </h2>
            </div>
            <Row className=''>
              {
                characteristics && characteristics?.map((cur, i) => (
                  <Col lg={6} md={12} xs={12} key={i} className='p-[30px] lg:p-[15px] flex items-center spesfics '>
                    <div className="flex p-[30px] md:p-[15px] md:flex-col md:items-center ">
                      <div>
                        <LazyLoadImage src={cur?.imgblue} className='current md:h-[100px] md:mb-5' />
                        <LazyLoadImage src={cur?.imgwhite} className='onhover hidden md:h-[100px] md:mb-5' />
                      </div>
                      <div className="flex flex-col pl-[30px] md:items-center md:pl-[0]">
                        <h2 className='text-[25px] font-[600] text-[--text] '>{cur?.text1}</h2>
                        <div className='mt-[20px] text-justify text-[--text]'>{cur?.text2}</div>
                      </div>
                    </div>
                  </Col>
                ))
        ...
        <section id="projects" className='pt-[50px] pb-[50px] bg-[--bg]'>
          <Container>
            <div className="w-full flex items-center mb-[30px]">
              <LazyLoadImage src={line} className='h-[2px] mr-[20px]' />
              <h2 className='text-[20px] font-bold text-[--text] uppercase'>projects </h2>
            </div>
            <Row className=''>
              {
                myprojects && myprojects?.slice(0, 4)?.map((cur, i) => (
